...
function solution(input) {
  ...
  const point = [1, ...input[E + 1].split(' ').map(Number)];
  const answer = [[]];

  for (let i = 0; i < 3; i++) {
    let heap = new PriorityQueue();
    heap.enqueue([0, point[i]], 0);
    ...
    answer[point[i]] = visited;
  }

  let res1 =
    answer[1][point[1]] + answer[point[1]][point[2]] + answer[point[2]][N];
  let res2 =
    answer[1][point[2]] + answer[point[2]][point[1]] + answer[point[1]][N];

  console.log(Math.min(res1, res2));
}
